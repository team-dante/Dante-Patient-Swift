...
//  Dante Patient
//
//  Created by Xinhao Liang on 9/7/19.
...
class WeekGraphViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    
    var startOfWeek = ""
    
    @IBOutlet weak var weekStr: UILabel!
    ...
    func changeDateLabel() {
        self.endOfWeek = self.parseEndOfWeek(date: startOfWeek)
        let parsedWeekStr = self.parseWeek(firstDay: self.startOfWeek, lastDay: self.endOfWeek)
        self.weekStr.text = parsedWeekStr
    }
    ...
    func loadData() {
        ...
        ref.child("/PatientVisitsByDates/\(userPhoneNum!)/").queryStarting(atValue: nil, childKey: self.startOfWeek)
            .observeSingleEvent(of: .value, with: { (snapshot) in
                ...
                if let dateObjs = snapshot.value as? [String: Any] {
                    ...
                    for dateObj in dateObjs {
                        ...
                        // if date is earlier than the end of week
                        if key < self.endOfWeek {
                            visitCount += 1
                            if let timeObjs = dateObj.value as? [String: Any] {
                                ...
                                weekDict.append(newdict)
                            }
                        }
                    }
                    // weekDict structure: [[("femaleWaitingRoom", 111), ("exam1", 222)],
                    //                       [("CTRoom", 333), ("exam1", 444)]]
                    for data in weekDict {
                        for room in data {
